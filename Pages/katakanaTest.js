...
function primoKana() 
{
    var immagine = document.getElementById("KatakanaPicture"); //creo l'elemento immagine
    var kanaRisultato = kana[kanaCasuale()]; //utilizzo il metodo kanaCasuale per avere un kana random
    var risultato = `../Images/Katakana/${kanaRisultato}.png`; //ho l'indirizzo dell'immagine del kana random che ho generato prima
...
function controllaTesto() {
    ...
    var kanaIndirizzo = document.getElementById("KatakanaPicture").src; //ottengo l'indirizzo dell'immagine attualmente utilizzata
    ...
    var immagine = document.getElementById("KatakanaPicture"); //copio l'immagine per modificarla con canva

    var immagineNormale = new Image();
    immagineNormale.src = `../Images/Katakana/${kanaImmagine}.png`; //creo una copia della vecchia immagine per poterla riapplicare dopo
    ...
    if (testoInserito.toLowerCase() === kanaImmagine.toLowerCase()) { //controllo che il testo iserito (dopo averlo trasformato in lowercase per comodita) sia uguale al kana rappresentato (sempre in lowercase per sicurezza)
        ...
        if(!kanaCorretti.includes(kanaImmagine) && !kanaSbagliati.includes(kanaImmagine)) //se kanaCorretti non contiene kanaImmagine e kanaSbagliati non contiene kanaImmagine
        {
            ...
            immagine.src = `../Images/Katakana/${kanaImmagine}_green.png`; //cambio l'immagine con quella verde
    ...
    else //altrimenti
    {
        ...
        if(!kanaSbagliati.includes(kanaImmagine)) //se kanaSbagliati non contiene kanaImmagine
        {
            ...
            immagine.src = `../Images/Katakana/${kanaImmagine}_red.png`; //cambio l'immagine con quella rossa
